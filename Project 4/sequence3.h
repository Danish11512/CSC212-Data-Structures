...
namespace data_structures_assignment3
{
    ...
    private:
        ...
        size_type many_nodes; // counter
        node* head_ptr; // not actual node just the head pointer
        node* tail_ptr; // not actual node helps with the attach function
        node* cursor; // not actual node, just the cursor
        node* precursor; //  not actual node the one behind the cursor to help with the insert function
